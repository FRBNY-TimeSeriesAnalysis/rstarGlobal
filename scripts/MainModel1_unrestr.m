%% MainModel1_unrestr.m    Estimates the "Baseline Model", modified to allow
%                          loadings for real rate to vary.

%% Initial setup

clear;

filename = '../results/OutputModel1_unrestr.mat';  % Output filename
...
Ccyc          = zeros(n, n*p);
Ccyc(1:n,1:n) = eye(n);  % Country/series-specific cyclic components
C             = [Ctr Ccyc];
...
%             r_wrd     pi_wrd      ts_wrd    cy_idio           pi_idio            ts_idio
SC0tr =    ([ 1         sqrt(2)     1         1*ones(1,7)       sqrt(2)*ones(1,7)  1*ones(1,7) ]).^2/100;
S0tr =      [ 0.5       2           1         zeros(1,7)        zeros(1,7)         zeros(1,7) ]';
P0tr = diag([ 1         2           1         1*ones(1,7)/2     2*ones(1,7)/2      1*ones(1,7)/2 ].^2);

%                stir          infl        ltir
Psi =       (2*[ ones(1,7)   2*ones(1,7)   ones(1,7) ]).^2;

S0cyc = zeros(n*p,1);  % Cyclic component
...
for jm = 1:Ndraws  % MCMC draws
    ...
    theta_old = [];
    theta_old = [theta_old; C(1:7,1)] ;  % Adding the loadings for r_wrd
    ...
    %adjusting loadingns to r_wrd
    C_new(1:7,  1) = theta_new(1:7);
    C_new(15:21,1) = theta_new(1:7);
    
    %adjusting Inflation to pi_wrd
    C_new(1:7,  2) = theta_new(8:14);
    C_new(8:14, 2) = theta_new(8:14);
    C_new(15:21,2) = theta_new(8:14);
